...
from bot.kb import chief_technician_tech_req_staff as cts
from bot.kb import (
...
@router.callback_query(F.data == cts.chief_technician_tech_req_button.callback_data)
async def chief_technician_tech_req_cb(callback: CallbackQuery):
    await try_edit_or_answer(
        message=callback.message,
        text=hbold("Тех. заявки"),
        reply_markup=cts.chief_technician_tech_req_rm,
    )


async def chief_technician_tech_req_ms(message: Message):
    await try_edit_or_answer(
        message=message,
        text=hbold(cts.chief_technician_tech_req_button.text),
        reply_markup=cts.chief_technician_tech_req_rm,
    )


@router.callback_query(
    F.data == cts.chief_technician_tech_req_change_department_button.callback_data
)
...
async def set_department(message: Message, state: FSMContext):
    deparments = get_deparments_by_repairman_telegram_id(message.chat.id)
    if await handle_department(
        message=message,
        state=state,
        departments=deparments,
        reply_markup=cts.chief_technician_tech_req_menu_markup,
    ):
        await chief_technician_tech_req_ms(message)


# region Chief technician own requests
@router.callback_query(
    F.data == cts.chief_technician_tech_req_own_button.callback_data,
)
async def chief_technician_tech_req_own(callback: CallbackQuery, state: FSMContext):
    department_name = (await state.get_data()).get("department_name")
    await try_edit_or_answer(
        message=callback.message,
        text=hbold(f"Производство: {department_name}"),
        reply_markup=cts.chief_technician_tech_req_own_menu_markup,
    )
...
async def chief_technician_tech_req_own_waiting(
    ...
    await try_edit_or_answer(
        ...
        reply_markup=create_keybord_for_requests_with_end_point(
            ...
            menu_button=cts.chief_technician_tech_req_own_menu_button,
            requests=requests,
        ),
    )
...
async def chief_technician_show_own_waiting_form_cb(
    ...
    await show_form(
        ...
        state=state,
        buttons=buttons,
        history_or_waiting_button=cts.chief_technician_tech_req_own_waiting,
    )


async def chief_technician_show_own_waiting_form_ms(
    ...
    await try_edit_or_answer(
        ...
        reply_markup=await cts.chief_technician_repair_tech_req_kb(
            state=state,
            callback_data=ShowRequestCallbackData(
...
async def set_repairman_photo(message: Message, state: FSMContext):
    await handle_documents(
        message, state, "photo", chief_technician_show_own_waiting_form_ms
    )
...
async def save_chief_technician_repair(
    callback: CallbackQuery, state: FSMContext, callback_data: ShowRequestCallbackData
):
    data = await state.get_data()
...
async def chief_technician_repair_form(
    callback: CallbackQuery, state: FSMContext, callback_data: ShowRequestCallbackData
):
    await try_edit_or_answer(
        message=callback.message,
...
@router.callback_query(
    ShowRequestCallbackData.filter(F.end_point == "chief_technician_show_form_history")
)
async def show_chief_technician_own_history_form(
    callback: CallbackQuery, state: FSMContext, callback_data: ShowRequestCallbackData
):
    buttons: list[list[InlineKeyboardButton]] = []
    await show_form(
        ...
        state=state,
        buttons=buttons,
        history_or_waiting_button=cts.chief_technician_tech_req_own_history,
    )
