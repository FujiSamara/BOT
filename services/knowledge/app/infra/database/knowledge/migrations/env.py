...
from app.infra.database.dish import models as extra_models

from alembic import context

settings: Settings = generate(Settings, logger)
config = context.config
config.set_main_option("sqlalchemy.url", settings.knowledge_ppsql_dsn)
...
def do_run_migrations(connection: Connection) -> None:
    ...
    connection.execute(text('set search_path to "%s"' % settings.knowledge_psql_schema))
