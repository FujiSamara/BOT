...
Revision ID: 45966bf300e5
Revises: 5da535571678
Create Date: 2024-10-14 17:19:33.429980
...
def upgrade() -> None:
    ...
    op.add_column("expenditures", sa.Column("kru_id", sa.Integer(), nullable=False))
    op.drop_constraint(
        "expenditures_cc_supervisor_id_fkey", "expenditures", type_="foreignkey"
    )
    op.create_foreign_key(None, "expenditures", "workers", ["kru_id"], ["id"])
...
def downgrade() -> None:
    ...
    op.drop_column("expenditures", "kru_id")
    op.add_column(
        "bids",
        sa.Column(
            "cc_supervisor_state",
