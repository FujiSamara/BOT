    ...
    with session.begin() as s:
        ...
        raw_bids = (
            ...
            .filter(
                and_(
                    Bid.teller_cash_state == ApprovalStatus.pending_approval,
                    Bid.department_issue_id == department_id,
                )
            )
            .all()
        )
...
def update_bid(bid: BidSchema, department_issue_name: Optional[str] = None):
    ...
    with session.begin() as s:
        ...
        if department_issue_name is None:
            cur_bid.department_issue = cur_bid.department_issue
            cur_bid.department_issue_id = cur_bid.department_issue_id
        else:
            department = (
                s.query(Department)
                .filter(Department.name == department_issue_name)
                .first()
            )
            cur_bid.department_issue = department
            cur_bid.department_issue_id = department.id
